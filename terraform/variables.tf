...
# These three values are generated by configure_scheduled_destroy.py. Keeping
# the opt-in Boolean separate lets Terraform safely use it for resource counts,
# while the contact object remains redacted in plans and command output.
variable "scheduled_destroy_enabled" {
  description = "Create the optional AWS-hosted scheduled self-destruct control plane."
  type        = bool
  default     = false
}

variable "scheduled_destroy_configuration" {
  description = "Generated non-secret schedule, backend, SMS routing, and content-addressed source settings."
  type = object({
    schedule_id                   = optional(string, "")
    deadline_utc                  = optional(string, "")
    deadline_epoch                = optional(number, 0)
    local_deadline                = optional(string, "")
    local_timezone                = optional(string, "")
    sms_origination_number        = optional(string, "")
    sms_inbound_topic_arn         = optional(string, "")
    source_bucket                 = optional(string, "")
    backend_key                   = optional(string, "")
    backend_region                = optional(string, "")
    terraform_source_key          = optional(string, "")
    terraform_source_sha256       = optional(string, "")
    controller_source_key         = optional(string, "")
    controller_source_sha256      = optional(string, "")
    controller_source_hash_base64 = optional(string, "")
    runner_actions                = optional(list(string), [])
  })
  default = {}

  validation {
    condition = !var.scheduled_destroy_enabled || (
      can(regex("^[a-f0-9]{16}$", var.scheduled_destroy_configuration.schedule_id)) &&
      can(regex("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$", var.scheduled_destroy_configuration.deadline_utc)) &&
      var.scheduled_destroy_configuration.deadline_epoch > 0 &&
      can(regex("^\\+[1-9][0-9]{7,14}$", var.scheduled_destroy_configuration.sms_origination_number)) &&
      can(regex("^arn:[^:]+:sns:[a-z0-9-]+:[0-9]{12}:[^:]+$", var.scheduled_destroy_configuration.sms_inbound_topic_arn)) &&
      length(var.scheduled_destroy_configuration.source_bucket) > 2 &&
      length(var.scheduled_destroy_configuration.backend_key) > 0 &&
      length(var.scheduled_destroy_configuration.runner_actions) > 0
    )
    error_message = "Enabled scheduled self-destruct settings must come from scripts/configure_scheduled_destroy.py."
  }
}

variable "scheduled_destroy_contacts" {
  description = "Generated operator phone and email. These values are sensitive and are also stored in encrypted Terraform state."
  type = object({
    operator_phone = optional(string, "")
    operator_email = optional(string, "")
  })
  default   = {}
  sensitive = true
  ...
  validation {
    condition = !var.scheduled_destroy_enabled || (
      can(regex("^\\+[1-9][0-9]{7,14}$", var.scheduled_destroy_contacts.operator_phone)) &&
      can(regex("^[^@[:space:]]+@[^@[:space:]]+\\.[^@[:space:]]+$", var.scheduled_destroy_contacts.operator_email))
    )
    error_message = "Enabled scheduled self-destruct requires a valid E.164 cell number and email address."
  }
}
